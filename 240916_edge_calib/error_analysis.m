%-------------------------------------------------
%
% ERROR_ANALYSIS - Performs Procrustes analysis to compare estimated and real coordinates.
% This script loads ground truth and estimated coordinates, aligns them using
% Procrustes analysis, calculates the RMSE, and plots the results for comparison.
%
% Inputs:
%   None
%
...
%   - calibration_output_6dof.mat
%   - calibration_output_7dof.mat
%
% Author: Tiago Simões
% Project: VIENA
...
function error_analysis()
    ...
    config.path_edge = 'calibration_output_6dof.mat'; % Edge-based estimation (without FOV)
    config.path_edge_fov = 'calibration_output_7dof.mat'; % Edge-based estimation (with FOV)
    config.path_error_results = 'error_results.mat';

    % Load the necessary coordinate data files into the config struct.
    load(config.path_real, 'real_coords');
    state.real_coord = real_coords; 
    load(config.path_edge, 'edge_output');
    state.edge_coord = vertcat(edge_output(:).car_position);
    state.edge_coord = state.edge_coord(:, 1:2);
    load(config.path_edge_fov, 'edge_output');
    state.edge_fov_coord = vertcat(edge_output(:).car_position);
    state.edge_fov_coord = state.edge_fov_coord(:, 1:2); 
    ...
    plot_results(state);
    save_results(config.path_error_results, state);
end
...
% (Edge, Edge-FOV) to the real-world ground truth coordinates.
%
% Inputs:
